						...
						$uidFirst=admin::getDbValue("SELECT distinct  a.pca_uid FROM mdl_pro_category a, mdl_subasta b where b.sub_pca_uid=a.pca_uid and b.sub_status='ACTIVE' order by pca_name limit 1");
						 $sWhere=" and pca_uid=$uidFirst";
							...
							while ($content=$db2->next_record())
							{
								...
								//echo "select count(*) from mdl_incoterm where inc_sub_uid=".$content["sub_uid"]." and inc_cli_uid=".admin::getSession("uidClient");
								...
								if($valIcoterms==0)
								{
									...
									else
									{
										...
										$valTerminos = admin::getDbValue("select count(*) from mdl_terminos where ter_sub_uid=".$content["sub_uid"]." and ter_cli_uid=".admin::getSession("uidClient"));
											...
											if($valTerminos ==0)
											{
											$token = SymmetricCrypt::encrypt(base64_encode($domain.'/'.$CategoryUrl.'/'.$content["pca_url"].'/'.$content["pro_url"].'/'));
											$_keycode = SymmetricCrypt::encrypt(base64_encode($content["sub_uid"]));
											$urlLink = $domain.'/code/reglamento_subasta.php?token='.$token.'&_keycode='.$_keycode.'&" rel="facebox';
											}else
											{
