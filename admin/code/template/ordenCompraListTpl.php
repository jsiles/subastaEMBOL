...
if (!$search || $search=='')
{
$_pagi_sql= "select * from mdl_orden_compra, mdl_client where orc_cli_uid=cli_uid and orc_delete=0 order by orc_uid asc ";
$nroReg=admin::getDBvalue("select count(*) from mdl_orden_compra where orc_cli_uid=cli_uid and orc_delete=0");
}
else
{
$_pagi_sql= "select * from mdl_orden_compra where orc_cli_uid=cli_uid and  orc_delete=0 and ((cli_socialreason like '%$search%')or(orc_date like '%$search%')) order by sol_uid asc ";
$nroReg=admin::getDBvalue("select count(*) from mdl_orden_compra where orc_delete=0 and ((cli_socialreason like '%$search%')or(orc_date like '%$search%')) ");
}
